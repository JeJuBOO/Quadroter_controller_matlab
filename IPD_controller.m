function [input_omega,Thrust,torq] = IPD_controller(s,s_D,param,~,acc) 
%input data : 
...
d(1) = K_P_pos(1).*(s_D.pos(1)-s(1)) + K_D_pos(1).*(s_D.vel(1)-s(4)) + K_DD_pos(1).*(s_D.acc(1)-acc(1));

d(2) = K_P_pos(2).*(s_D.pos(2)-s(2)) + K_D_pos(2).*(s_D.vel(2)-s(5)) + K_DD_pos(2).*(s_D.acc(2)-acc(2));

d(3) = K_P_pos(3).*(s_D.pos(3)-s(3)) + K_D_pos(3).*(s_D.vel(3)-s(6)) + K_DD_pos(3).*(s_D.acc(3)-acc(3));
